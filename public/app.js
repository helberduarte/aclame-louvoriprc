...
function renderNav() {
  const itens = [];
  itens.push(['secao', 'Igreja']);
  itens.push(['#/mural', '📢 Mural']);
  itens.push(['#/estante', '🎵 Estante musical']);
  itens.push(['secao', 'Minha área']);
  itens.push(['#/agenda', '📅 Minha agenda']);
  itens.push(['#/disponibilidade', '🗓️ Disponibilidade']);
  itens.push(['#/habilidades', '🎯 Habilidades']);
  itens.push(['#/trocas', '🔄 Trocas']);
  itens.push(['#/pontos', '🏆 Pontos']);
  itens.push(['#/notificacoes', '🔔 Notificações']);
  if (ehLider()) {
    itens.push(['secao', 'Gestão']);
    itens.push(['#/dashboard', '📊 Visão geral']);
    itens.push(['#/escala-mensal', '⚡ Escala do mês']);
    itens.push(['#/escalas', '🗂️ Cultos & escalas']);
    itens.push(['#/indisponibilidades', '🚫 Indisponibilidades']);
    itens.push(['#/eventos', '⛪ Celebrações']);
    itens.push(['#/ministerios', '🏛️ Ministérios']);
    itens.push(['#/voluntarios', '🧑‍🤝‍🧑 Voluntários']);
    itens.push(['#/feedbacks', '⭐ Feedbacks']);
  }
  if (ehAdmin()) {
    itens.push(['secao', 'Administração']);
    itens.push(['#/usuarios', '🔐 Usuários & papéis']);
  }
  const hash = location.hash || '#/mural';
  document.getElementById('sidebar').innerHTML = itens.map(([h, rotulo]) =>
    h === 'secao' ? `<div class="secao">${rotulo}</div>` :
      `<a href="${h}" class="${hash.startsWith(h) ? 'ativa' : ''}">${rotulo}</a>`).join('');
}
